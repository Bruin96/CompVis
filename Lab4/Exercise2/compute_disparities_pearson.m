    ...
    for x = x_bottom:x_top
        ...
        for y = y_bottom:y_top
            ...
            for d = 1:-lower_bound(x,y)+1
                ...
                costs(x, y, -(d+lower_bound(x,y)-2)) = (win_height*win_width*sum(sum(im1_window.*im2_window)) - sum(sum(im1_window))*sum(sum(im2_window))) / (sqrt(win_height*win_width*sum(sum(im1_window.^2)) - (sum(sum(im1_window))).^2) * sqrt(win_height*win_width*sum(sum(im2_window.^2)) - (sum(sum(im2_window))).^2));
            end
        end
    end   
