...
function dispars = compute_disparities(im1, im2, method)
    if strcmp(method, 'XCORR') == 1
        dispars = compute_disparities_cross_correlation(im1, im2);
    elseif strcmp(method, 'SSD') == 1
        dispars = compute_disparities_sum_squared_diff(im1, im2);
    elseif strcmp(method, 'AD') == 1
        dispars = compute_disparities_abs_diff(im1, im2);
    elseif strcmp(method, 'SNR') == 1
        dispars = compute_disparities_signal_to_noise(im1, im2);
    elseif strcmp(method, 'PEARSON') == 1
        dispars = compute_disparities_pearson(im1, im2);
    else
